...
def get_telegraph() -> Telegraph:
    ...
    if _telegraph is None:
        try:
            ...
            print(f"⚠️ Telegraph create_account failed: {e}")
            raise
    return _telegraph
...
def publish_to_telegraph(
    ...
    """Publish to Telegraph - nhận CuratedNewsletter hoặc raw HTML."""
    if curated:
        telegraph_content = render_telegraph_html(curated)
    else:
...
def render_telegraph_html(curated) -> str:
    """Tạo HTML chuyên nghiệp dành cho Telegraph."""
    import html
    
    # Nếu nhận string (HTML content) thì bỏ qua
    if isinstance(curated, str):
        return ""
    
    banner_url = "https://images.unsplash.com/photo-1677442136019-21780ecad995?q=80&w=1000&auto=format&fit=crop"
    
    parts = []
    
    # Banner
    parts.append(f'<img src="{banner_url}">')
    
    # Lead
    if hasattr(curated, 'lead') and curated.lead:
        parts.append(f"<p>{html.escape(curated.lead)}</p>")
    
    # Repos
    if hasattr(curated, 'repos') and curated.repos:
        parts.append("<hr>")
        parts.append("<h3>🔥 TOP GITHUB TRENDING</h3>")
        
        for i, repo in enumerate(curated.repos, 1):
            safe_title = html.escape(repo.title)
            safe_url = html.escape(repo.url, quote=True) if repo.url else ""
            
            title_html = f'<a href="{safe_url}">{safe_title}</a>' if safe_url else safe_title
            parts.append(f"<h4>{i}. {title_html}</h4>")
            
            if repo.tldr:
                parts.append(f"<blockquote>{html.escape(repo.tldr)}</blockquote>")
            
            if repo.why_it_matters:
                parts.append(f"<p><b>💡 Ứng dụng:</b> {html.escape(repo.why_it_matters)}</p>")
            
            if repo.highlights:
                parts.append("<ul>")
                for hl in repo.highlights:
                    parts.append(f"<li>{html.escape(hl)}</li>")
                parts.append("</ul>")
    
    # Papers
    if hasattr(curated, 'papers') and curated.papers:
        parts.append("<hr>")
        parts.append("<h3>📄 NGHIÊN CỨU & PAPERS</h3>")
        
        for i, paper in enumerate(curated.papers, 1):
            safe_title = html.escape(paper.title)
            safe_url = html.escape(paper.url, quote=True) if paper.url else ""
            
            title_html = f'<a href="{safe_url}">{safe_title}</a>' if safe_url else safe_title
            parts.append(f"<h4>{i}. {title_html}</h4>")
            
            if paper.tldr:
                parts.append(f"<blockquote>{html.escape(paper.tldr)}</blockquote>")
            
            if paper.why_it_matters:
                parts.append(f"<p><b>🎯 Giá trị:</b> {html.escape(paper.why_it_matters)}</p>")
    ...
    # Footer
    parts.append("<hr>")
    parts.append("<p><i>🤖 Generated by <b>AI News Agent</b></i></p>")
